...
// 设置用户ID，以识别不同登录用户，未设此值sdk将默认生成一个
config.setUid("android_sdk_test_id");
// 设置用户VIP等级(0-5)config.setVip(0);
...
当 `nickname`、`uid`、`avatar`、`device_token` 等配置项配置完下次需要更新时需要调用 **`V5ClientConfig.getInstance(context).shouldUpdateUserInfo()`**，这样才会向服务端更新这几个配置项。
...
## 6 使用SDK接口开发
> 注: 建议使用 SDK 提供的 UI 库直接集成，不建议使用 SDK 接口开发，因客服消息包含文本、图片、语音、图文等多种类型，单独开发难度较大。
...
//【建议】设置用户ID，区分APP登录的不同账号，未设置则由SDK生成随机uidconfig.setUid(String uuid); 
// 设置性别 0-未知 1-男 2-女config.setGender(1); 
// 设置用户VIP等级(0-5) config.setVip(0); // 设置device_token:集成第三方推送(腾讯信鸽、百度云推)时设置此参数以在离开会话界面时接收推送消息
config.setDeviceToken(String device_token);//当nickname、uid、avatar、device_token等配置项配置完下次需要更新时
